...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i <= iters; i++) {
        ...
        int bad = immediate != 0 || except_ready || !write_ready || after != 0;
        if (bad && bad_immediate + bad_except + bad_write + bad_after < 10) {
            printf(
                "iter=%d connect_rc=%d immediate=%d select_rc=%d read=%d write=%d except=%d after=%d\n",
                i,
                connect_rc,
                immediate,
                select_rc,
    ...
    printf(
        "summary bad_immediate=%d/%d bad_except=%d/%d bad_missing_write=%d/%d bad_after=%d/%d child_status=%d\n",
        bad_immediate,
        iters,
        bad_except,
        iters,
    ...
    return bad_immediate == 0 && bad_except == 0 && bad_write == 0 && bad_after == 0 && status == 0
               ? 0
               : 1;
}
